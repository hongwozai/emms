...
function add()
{
form.action="../application/insertpkey.php";  
}
function reset()
{
 form.action="plugkey.php";
}
</script>
</head>
<body>
<div class="wrap J_check_wrap">
     ...
     <option value="5">附件内容</option> 
		...
		<tr>
		<input class="input" id="mkey" required name="pkey" type="text" placeholder="封堵关键字内容" title="封堵关键字内容" value=""/>		
		</tr>
        </td>
        <td>
      ...
      <table class="table" style="border:1px solid #ddd;" align="center" id="mytable">
		...
		<?php 
		 include '../conn.php';
		/* $stmt = oci_parse($conn, "select * from KILL_KEY");
		 ...
		 $sql_exc_page = OCIParse($conn,"select * from KILL_KEY order by KILL_ID desc");
		 OCIExecute($sql_exc_page);
		 $toltalnum = oci_fetch_all($sql_exc_page,$result);
		 $totalpage = ceil($toltalnum/$perNumber);
		 $sql_fenye = "select * from KILL_KEY order by KILL_ID desc";
		 ...
		 $sql_fenye = "select * from KILL_KEY  where KILL_ID <= $mID  order by KILL_ID desc";
		...
		?>
		 ...
		 <li align="right">
        <a href="../frame/plugkey.php?page=<?php echo 1;?>" target="_self">首页</a>
        <?php 
        if($page != 1)		
        {
        ?>
        <a href="../frame/plugkey.php?page=<?php echo $page - 1;?>" target="_self">上页 </a>
        <?php  
        }
        if ($page < $totalpage )
        {
        ?>
        <a href="../frame/plugkey.php?page=<?php echo $page +1; ?>" target="_self">下页</a>
        <?php	
        }
        ?>
        <a href="../frame/plugkey.php?page=<?php echo $totalpage;?>" target="_self">尾页 </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</li>          
