...
<div class="wrap">
		...
		<form name="myform" action="../application/mkeyshow.php" method="post">
...
<?php 
     ...
     // $type=$_POST['number'];
      ...
      if($_SESSION['username']){
       ...
       /* $stmt = oci_parse($conn, "select * from MONITOR_KEY where TYPE='$type'");
        oci_define_by_name($stmt, "KEYWORD", $keyword);
        oci_define_by_name($stmt, "TYPE", $type);
        oci_define_by_name($stmt, "MONITOR_ID", $id);
        oci_execute($stmt);*/
        $sql_exc_page = OCIParse($conn,"select * from MONITOR_KEY where TYPE='$type' order by MONITOR_ID desc");
        OCIExecute($sql_exc_page);
        $toltalnum = oci_fetch_all($sql_exc_page,$result);
        $totalpage = ceil($toltalnum/$perNumber);
        $sql_fenye = "select * from MONITOR_KEY where TYPE='$type' order by MONITOR_ID desc";
        ...
        $sql_fenye = "select * from MONITOR_KEY where TYPE='$type' and MONITOR_ID <= $mID  order by MONITOR_ID desc";
        ...
        oci_execute($sql_exc);
         for ($i = 0; $i < $perNumber; $i++){
            ...
            <td style="border:1px solid #ddd;" width=100px><?php echo $row['KEYWORD'];?></td>
              <td style="border:1px solid #ddd;" width=100px><a href="../application/delm.php?postid=<?php echo $row['MONITOR_ID'];?>&numb=<?php echo $row['TYPE'];?>">删除</a></td>
             </tr>
         <?php } 
      if($row['TYPE']==1){ ?> 
             <tr>
...
?>
    ...
    <li align="right">
        <a href="../application/mkeyshow.php?page=<?php echo 1;?>&number=<?php echo $type;?>" target="_self">首页</a>
        <?php 
        if($page != 1)		
        {
        ?>
        <a href="../application/mkeyshow.php?page=<?php echo $page - 1;?>&number=<?php echo $type;?>" target="_self">上页 </a>
        <?php  
        }
        if ($page < $totalpage )
        {
        ?>
        <a href="../application/mkeyshow.php?page=<?php echo $page +1; ?>&number=<?php echo $type;?>" target="_self">下页</a>
        <?php	
        }
        ?>
        <a href="../application/mkeyshow.php?page=<?php echo $totalpage;?>&number=<?php echo $type;?>" target="_self">尾页 </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</li>          
